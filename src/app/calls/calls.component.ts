...
export class CallsComponent  implements OnInit {
  ...
  balloonStatus:boolean=false;
  getPhonePosition(i:number) {
    if(this.balloonStatus==false){
      if(i==6){
        let phone6:HTMLElement|null = document.querySelector("#phone6");
        if(phone6 && phone6.offsetTop>0){
          let content:HTMLElement|null = document.querySelector("ion-tabs");
          if(content){
            let top = phone6.offsetTop;
            let left = phone6.offsetLeft + content.offsetLeft;
            this.tutorial.balloonsStatus.next({top:top,left:left});
            this.balloonStatus=true;
          }
        }
      }
    }
